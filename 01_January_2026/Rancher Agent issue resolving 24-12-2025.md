...
journalctl -u k3s -n 50 						# Hard to read, so i exported into a file
touch /tmp/k3s-journal.txt
journalctl -u k3s -n 50 > /tmp/k3s-journal.txt
journalctl -u k3s -n 20 | grep 'error' 
netstat -tulnp | grep '6443'
iptables -L -n -v | grep '10.100.93.6'
...
>>The etcd error: "failed to publish local member to cluster through raft"

<<<< Try Sollution 1: allow etcd communication with other VMs >>>>>
# Allow etcd communication from 93.7
iptables -I INPUT -p tcp -s 10.100.93.7 --dport 2379:2380 -j ACCEPT
iptables -I INPUT -p udp -s 10.100.93.7 --dport 8472 -j ACCEPT
