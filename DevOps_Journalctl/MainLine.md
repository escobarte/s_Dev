...
#### На прошлой неделе **Планировал** закончить Ci/CD Docker Compose on Laravel.

Почему не получилось? Нету понятия как устроен Docker Compose. Начал с нуля работу с докер компоуз. Шаг за шагом установку на сервер, и установку Laravel Framework через него. Сегодня 25 Августа, продолжаю делать. Также понял, что нужно пройти быстрое внедрение в Laravel.

## Day 26 August [Continues Integragtion in Dev Ops]

Вчера шаг за шагом, сделал docker compose с 2 (nginx && php)  сервисами с 1 билд php-fpm. Было не ясно - но в конченом итоге вывел PHP info. Сегодня продолжаю. Пора уже довести до конца.
Сегодня я довёл до конца Deploy docker compose laravel. С поврторением. Есть вероятность что понял.
Следующий шаг CI/CD Laravel Docker

## Day 28 August [Continues Integragtion in Dev Ops]

Нашёл хорошое видео обучение по CI/CD Laravel. Пересмотрел раз. Что-то понял. Взялся делать как он предлогает. Опять спотыкался об .git.
Много пуршил - короче. Вроде пошёл шаг за шагом, и что я понял ? Мне кажется, он берет ужё развёрнутый и с рабочим ключём проект, и просто деплоит  его ? ... хуй-пойми. 

#### Ничего не вышло сегодня.

Я вообще запутался что я делаю, зачем мне и как этим пользоваться. ..... 
....

## Day 29 August [Continues Integragtion in Dev Ops]

Что я узнал: 
Я узнал что обычно делается так, сначала разварачивается проект на сервере, у меня он есть в двух варициях, (shell && docker-compose).
После этого когда сайт запущен и работат, происходит этам CI. То есть, это продукт - билдится в контейнер, чтобы в будующем его можно было развернуть на любой инстанции, минуя многие зависимости(ос, и тд). Сконцентрируюсь на это на данном этапе. 

## September 8 Day

У меня готовый, минимальный и простейший CI / CD.
Как он работает. 

1. Делаем Build используя Dockerfile.
2. Когда билд готов, разварачиваем его на Test-Prod Сервере.
3. Когда на тест всё ок, идёт всё на Prod.